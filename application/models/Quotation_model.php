...
class Quotation_model extends CI_Model
{
	...
	*/
	public function editModel($id,$data){
		...
		$sql = "update quotation set date = ?,reference_no = ?,warehouse_id = ?,customer_id = ?,biller_id = ?,total = ?,discount_value=?,tax_value=?,note = ?,shipping_city_id = ?,shipping_state_id= ?,shipping_country_id =?,shipping_address =?,shipping_charge =?,internal_note = ?,mode_of_transport=?,transporter_name=?,transporter_code=?,vehicle_regn_no=?,user = ? where quotation_id = ?";
		if($this->db->query($sql,$data)){
			return true;
		}
		else{
